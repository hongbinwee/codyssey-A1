...
| --- | --- | --- |
| 복수 지역 추천 | recommended_city를 2~3개로 확장 | 미구현 |
| 결과 캐싱 | 같은 `-date` 재실행 시 기존 JSON 활용 | 구현 완료(선택 보너스) |

복수 지역 추천은 현재 범위에 포함하지 않는다. 결과 캐싱은 같은 날짜의 완전한 raw JSON을 재사용하고, Markdown이 없거나 필수 섹션이 빠진 경우 fallback Markdown을 재생성한다.
